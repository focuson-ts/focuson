...
import { commonTagFetchProps, HasTagHolder, pageAndTagFetcher, updateTagsAndMessagesOnError } from "@focuson/fetcher";
...
export function searchFetcher<S extends SearchRequirements> () {
  return pageAndTagFetcher<S, string[], SimpleMessage> (
    commonTagFetchProps<S, string[]> ( ( s, date ) => [ createSimpleMessage ( 'info', s.toString (), date ) ], defaultDateFn ) ( updateTagsAndMessagesOnError ( data => [] ) ),
    'search',
    undefined,
    //@ts-ignore
