...
describe ( 'addModalPageIfNeeded', () => {
  ...
  it ( "to show only the main page if there is no modal page", () => {
    const state = lensState<PageSpecState> ( { messages: [], pageSelection: { pageName: 'nothing' , pageMode: "view"}, someData: 'x', modalData: 'y', }, () => {}, 'addModalPageIfNeeded' ).focusOn ( 'someData' )
  ...
  it ( "to show main page and modal if requested", () => {
    const state = lensState<PageSpecState> ( { messages: [], pageSelection: { pageName: 'nothing' , pageMode: "view" }, someData: 'x', modalData: 'y', currentSelectedModalPage: 'someModalPage' }, () => {}, 'addModalPageIfNeeded' ).focusOn ( 'someData' )
    const comp = shallow ( addModalPageIfNeeded ( modalPageDetails, state, DisplayPageSpecState ( { state } ) ) )
    expect ( comp.text () ).toEqual ( 'Main xModal y' )
  } )

  it ( "to throw error if illegal modal page selected", () => {
    const state = lensState<PageSpecState> ( { messages: [], pageSelection: { pageName: 'nothing'  , pageMode: "view"}, someData: 'x', modalData: 'y', currentSelectedModalPage: 'illegalName' }, () => {}, 'addModalPageIfNeeded' ).focusOn ( 'someData' )
