...
export function wrapWithErrors ( id: string, errors: string[][] | undefined, extraConditions: [ boolean, string ][], fn: ( props: ErrorhandlingProps, error: boolean, errorRef: React.MutableRefObject<HTMLUListElement>, allErrors: string[] ) => JSX.Element, maxErrors?: number ) {
  const actualMaxErrors = maxErrors ? maxErrors : 1
  ...
  return <span className='tooltip-wrap'>{fn ( props, error, ref, allErrors )}
    <ul ref={ref} hidden={true} className='errormessage tooltip' id={errorId}>{allErrors.slice ( 0, actualMaxErrors ).map ( enableByToLi )}</ul>
  </span>
}