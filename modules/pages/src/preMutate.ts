...
export const preMutateForPages = <S, Context extends PageSelectionContext<S>> ( c: Context ) => ( state: S ): S =>
  c.pageSelectionL.get ( state ).reduce <S> ( ( acc, pageSelection, i ) => premutateOnePage ( c, acc, i ), state )


function premutateOnePage<S, Context extends PageSelectionContext<S>> ( c: Context, s: S, i: number ): S {
  const lens = c.pageSelectionL.chainLens ( Lenses.nth ( i ) )
  const pageSelection = lens.get ( s )
  const { firstTime, pageName } = pageSelection
  ...
  if ( firstTime ) {
    ...
    if ( details.clearAtStart )
      return firstTimeLens.combine ( details.lens ).set ( s, [ false, undefined ] );
    if ( details.initialValue )
      return firstTimeLens.combine ( details.lens ).set ( s, [ false, details.initialValue ] );
