...
import { fromPathGivenState, HasPageSelectionLens, pageSelectionlens } from "@focuson/pages";
import { safeArray } from "@focuson/utils";


export function MakeTest<S, C> ( { state }: LensProps<S, any, C> ) {
  function onClick () {
    const traces = state.copyWithLens ( traceL () ).optJson ()
...
export function AssertState<S, C extends FocusOnContext<S>> ( { state }: LensProps<S, any, C> ) {
  function onChange ( e: any ) {
    try {
      let json = fromPathGivenState<S, C> ( state ) ( e.target.value ).getOption ( state.main );
