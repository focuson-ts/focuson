...
export function ActionButton<S, C extends FocusOnContext<S>> ( { id, state, action, text, enabledBy, paths, preCommands, postCommands }: ActionButtonProps<S, C> ) {
  ...
  function onClick ( e: any ) {
    ...
    const post: Transform<S, any>[] = processChangeCommandProcessor ( errorPrefix, processor, preCommands )
  ...
  return wrapWithErrors ( id, enabledBy, [], ( errorProps, error ) =>
    <button id={id} onClick={onClick} {...errorProps} disabled={error}>{text}</button> )
}