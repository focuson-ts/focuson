...
export const makeMockFetcherFor = ( params: JavaWiringParams ) => ( [ parent, action, dataD, resolver ]: [ DataD | undefined, RestActionDetail, AllDataDD, string ] ): string[] => {
  if ( parent && !action?.output?.needsObj )
    { // @ts-ignore
      ...
      return [ `    public DataFetcher ${resolverName ( parent, action )} (){ return new StaticDataFetcher("${sample}");}` ]
    }
  else
    return [ `    public DataFetcher ${resolverName ( dataD, action )}() {  return dataFetchingEnvironment -> ${sampleName ( dataD )}0();    }` ]

};

export function makeMockFetchers ( params: JavaWiringParams, rs: RestD[] ): string[] {
