...
function makeModalButtonFrom ( parent:PageD, name: string, button: ModalButtonInPage ): string {
  ...
  return `<${button.control} id='${name}' text='${name}' modal = '${modalName(parent, modal)}' state={state} ${opt ( 'mode', mode )} ${opt ( 'mainData', mainData )} ${opt ( 'tempData', tempData )} ${restOnCommitString ( restOnCommit )}  />`;
}
