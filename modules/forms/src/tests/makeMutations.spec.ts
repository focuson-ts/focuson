...
describe ( "makeMutations", () => {
  it ( "should create an mutation class with a method for each mutation for that rest - simple", () => {
    expect ( makeMutations ( paramsForTest, EAccountsSummaryPD, 'theRestName', eAccountsSummaryRestD, safeArray ( eAccountsSummaryRestD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "",
      "    public void auditStuff0(Connection connection, String dbName, int accountId, int clientRef) throws SQLException {",
  ...
  it ( "should create an mutation class with a method for each mutation for that rest - complex", () => {
    expect ( makeMutations ( paramsForTest, ChequeCreditbooksPD, 'theRestName', chequeCreditBooksRestD, safeArray ( chequeCreditBooksRestD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "",
      "    public Tuple2<Integer,String> sequencename0(Connection connection, Object dbName) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: sequencename0( {'type':'output','name':'checkbookId','javaType':'Integer','sqlType':'INTEGER'}, {'type':'output','name':'checkbookIdPart2','javaType':'String','sqlType':'CHAR'}, {'type':'autowired','name':'systemTime','class':'{thePackage}.utils.ITimeService','method':'now()','import':true}+ )\");",
      ...
      "  }}",
      "    public void auditCreateCheckBook1(Connection connection, Object dbName, int brandRef, int accountId, Object checkbookId, Object checkbookIdPart2) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: auditCreateCheckBook1( brandRef, accountId, checkbookId, checkbookIdPart2+ )\");",
      "           return;",
      "    }",
      ...
      "    public void manualLog2(Connection connection, Object dbName, Object checkbookId, Object checkbookIdPart2) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
  ...
  it ( "should make mutations for cases: i.e. where only one of several mutations will happen depending on the situation", () => {
    expect ( makeMutations ( paramsForTest, PaymentsPageD, 'newPayments', newPaymentsRD, safeArray ( newPaymentsRD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "//If you have a compiler error in the type here, did you match the types of the output params in your manual code with the declared types in the .restD?",
      "    public Tuple2<String,Integer> create0(Connection connection, Object dbName, int brandRef, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      ...
      "    }",
      "      if (true && brandRef==3) {",
      "        Tuple2<String,Integer> params0 = one0_0(connection,dbName,accountId);",
      ...
      "      }",
      "      if (true) {",
      "        Tuple2<Integer,String> params1 = two0_1(connection,dbName,accountId);",
      ...
      "  }",
      "    public Tuple2<String,Integer> one0_0(Connection connection, Object dbName, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: one0_0( {'type':'string','value':'first'}, accountId, {'type':'output','name':'one','javaType':'String','sqlType':'CHAR'}, {'type':'output','name':'two','javaType':'Integer','sqlType':'INTEGER'}+ )\");",
      "           return new Tuple2<>(\"0\",1);",
      "    }",
      ...
      "      return new Tuple2<String,Integer>(one,two);",
      "  }}",
      "    public Tuple2<Integer,String> two0_1(Connection connection, Object dbName, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
