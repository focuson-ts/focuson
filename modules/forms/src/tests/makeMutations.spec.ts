...
describe ( "makeMutations", () => {
  it ( "should create an mutation class with a method for each mutation for that rest - simple", () => {
    expect ( makeMutations ( paramsForTest, EAccountsSummaryPD, 'theRestName', eAccountsSummaryRestD, safeArray ( eAccountsSummaryRestD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "",
      "    public void auditStuff(Connection connection, String dbName, int accountId, int clientRef) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: auditStuff( {'type':'string','value':'someString'}, accountId, clientRef+ )\");",
      "           return;",
      "    }",
  ...
  it ( "should create an mutation class with a method for each mutation for that rest - complex", () => {
    expect ( makeMutations ( paramsForTest, ChequeCreditbooksPD, 'theRestName', chequeCreditBooksRestD, safeArray ( chequeCreditBooksRestD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "",
      "    public Tuple2<Integer,String> sequencename(Connection connection, Object dbName) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: sequencename( {'type':'output','name':'checkbookId','javaType':'Integer','sqlType':'INTEGER'}, {'type':'output','name':'checkbookIdPart2','javaType':'String','sqlType':'CHAR'}, {'type':'autowired','name':'systemTime','class':'{thePackage}.utils.ITimeService','method':'now()','import':true}+ )\");",
      ...
      "  }}",
      "    public void auditCreateCheckBook(Connection connection, Object dbName, int brandRef, int accountId, Object checkbookId, Object checkbookIdPart2) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: auditCreateCheckBook( brandRef, accountId, checkbookId, checkbookIdPart2+ )\");",
      "           return;",
      "    }",
      ...
      "    public void manualLog(Connection connection, Object dbName, Object checkbookId, Object checkbookIdPart2) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: manualLog( checkbookId, checkbookIdPart2+ )\");",
      "           return;",
      "    }",
  ...
  it ( "should make mutations for cases: i.e. where only one of several mutations will happen depending on the situation", () => {
    expect ( makeMutations ( paramsForTest, PaymentsPageD, 'newPayments', newPaymentsRD, safeArray ( newPaymentsRD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "",
      "@Autowired\n   public package1.package2.ClassName someVariable;",
      "//If you have a compiler error in the type here, did you match the types of the output params in your manual code with the declared types in the .restD?",
      "    public Tuple2<String,Integer> create(Connection connection, Object dbName, int brandRef, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: create( brandRef, accountId, {'type':'output','name':'one','javaType':'String'}, {'type':'output','name':'two','javaType':'Integer'}+ )\");",
      "           return new Tuple2<>(\"0\",1);",
      "    }",
      "      if (true && brandRef==3) {",
      "        Tuple2<String,Integer> params0 = one(connection,dbName,accountId);",
      ...
      "      }",
      "      if (true) {",
      "        Tuple2<Integer,String> params1 = two(connection,dbName,accountId);",
      ...
      "  }",
      "    public Tuple2<String,Integer> one(Connection connection, Object dbName, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: one( {'type':'string','value':'first'}, accountId, {'type':'output','name':'one','javaType':'String','sqlType':'CHAR'}, {'type':'output','name':'two','javaType':'Integer','sqlType':'INTEGER'}+ )\");",
      "           return new Tuple2<>(\"0\",1);",
      "    }",
      ...
      "      return new Tuple2<String,Integer>(one,two);",
      "  }}",
      "    public Tuple2<Integer,String> two(Connection connection, Object dbName, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: two( {'type':'string','value':'second'}, {'type':'output','name':'two','javaType':'Integer','sqlType':'INTEGER'}, {'type':'output','name':'one','javaType':'String','sqlType':'CHAR'}, accountId+ )\");",
      "           return new Tuple2<>(0,\"1\");",
      "    }",
