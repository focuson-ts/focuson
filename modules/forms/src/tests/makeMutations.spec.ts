...
describe ( "makeMutations", () => {
  ...
  it ( "should make mutations for cases: i.e. where only one of several mutations will happen depending on the situation", () => {
    expect ( makeMutations ( paramsForTest, PaymentsPageD, 'newPayments', newPaymentsRD, safeArray ( newPaymentsRD.mutations )[ 0 ] ) ).toEqual ( [
      ...
      "",
      "//If you have a compiler error in the type here, did you match the types of the output params in your manual code with the declared types in the .restD?",
      "    public Tuple2<String,Integer> oneLine_Payment_create_create0(Connection connection, Object dbName, int brandRef, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: oneLine_Payment_create_create0( brandRef, accountId, {'type':'output','name':'one','javaType':'String'}, {'type':'output','name':'two','javaType':'Integer'}+ )\");",
      "           return new Tuple2<>(\"0\",1);",
      "    }",
      "      if (true && brandRef==3) {",
      "        Tuple2<String,Integer> params0 = oneLine_Payment_create_one0_0(connection,dbName,accountId);",
      ...
      "      }",
      "      if (true) {",
      "        Tuple2<Integer,String> params1 = oneLine_Payment_create_two0_1(connection,dbName,accountId);",
      ...
      "  }",
      "    public Tuple2<String,Integer> oneLine_Payment_create_one0_0(Connection connection, Object dbName, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: oneLine_Payment_create_one0_0( {'type':'string','value':'first'}, accountId, {'type':'output','name':'one','javaType':'String','sqlType':'CHAR'}, {'type':'output','name':'two','javaType':'Integer','sqlType':'INTEGER'}+ )\");",
      "           return new Tuple2<>(\"0\",1);",
      "    }",
      ...
      "      return new Tuple2<String,Integer>(one,two);",
      "  }}",
      "    public Tuple2<Integer,String> oneLine_Payment_create_two0_1(Connection connection, Object dbName, int accountId) throws SQLException {",
      "        if (dbName.equals(IFetcher.mock)) {",
      "           System.out.println(\"Mock audit: oneLine_Payment_create_two0_1( {'type':'string','value':'second'}, {'type':'output','name':'two','javaType':'Integer','sqlType':'INTEGER'}, {'type':'output','name':'one','javaType':'String','sqlType':'CHAR'}, accountId+ )\");",
      "           return new Tuple2<>(0,\"1\");",
      "    }",
