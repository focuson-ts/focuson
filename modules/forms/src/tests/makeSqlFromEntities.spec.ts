  ...
  it ( "should make maps for each sql root, from the link data, for a 'single item'", () => {
    ...
    expect ( walkSqlRoots ( findSqlRoot ( jointAccountRestDTables ), ( r, path ) => {
      ...
      [
        ...
        "",
        "//{'accountId':{'rsSetter':'setInt','javaType':'int','graphQlType':'Int','typeScriptType':'number','javaParser':'Integer.parseInt','allowUndefined':false,'commonLens':'accountId','testValue':44444444},'brandRef':{'rsSetter':'setInt','javaType':'int','graphQlType':'Int','typeScriptType':'number','javaParser':'Integer.parseInt','allowUndefined':false,'commonLens':'brandRef','testValue':10},'dbName':{'rsSetter':'setString','javaType':'String','graphQlType':'String','typeScriptType':'string','javaParser':'','allowUndefined':false,'commonLens':'dbName','testValue':'mock'}}",
        ...
        "     return get(connection,msgs,accountId,brandRef,get0(connection,msgs,accountId,brandRef),get1(connection,msgs,accountId,brandRef)).map(x -> x._root);",
        "  }",
        ...
        "  ' order by mainCustomer.nameId\\n'+",
        "  '\\n';",
        "  public static Optional<JointAccount_jointAccountMaps> get(Connection connection, Messages msgs, int accountId, int brandRef, List<JointAccount_jointAccountMaps0> list0, List<JointAccount_jointAccountMaps1> list1) throws SQLException {",
        ...
        "        return Optional.of(new JointAccount_jointAccountMaps(rs,list0,list1));",
        "      } finally {",
        "        rs.close();",
        "        statement.close();",
        "      }",
        "  }",
        "  public static List<JointAccount_jointAccountMaps0> get0(Connection connection, Messages msgs, int accountId, int brandRef) throws SQLException {",
        ...
        "      }",
        "  }",
        "  public static List<JointAccount_jointAccountMaps1> get1(Connection connection, Messages msgs, int accountId, int brandRef) throws SQLException {",
  ...
  it ( "should make maps for a repeating item", () => {
    expect ( walkSqlRoots ( findSqlRoot ( addressRestDTables ), ( r, path ) => {
      ...
      [
        ...
        "import focuson.data.utils.Messages;",
        "",
        "//{'dbName':{'rsSetter':'setString','javaType':'String','graphQlType':'String','typeScriptType':'string','javaParser':'','allowUndefined':false,'commonLens':'dbName','testValue':'mock'},'postcode':{'rsSetter':'setString','javaType':'String','graphQlType':'String','typeScriptType':'string','javaParser':'','allowUndefined':false,'lens':'~/postcode/search','testValue':'LW12 4RG'}}",
        ...
        "     return get(connection,msgs,postcode).stream().map(x -> x._root).collect(Collectors.toList());",
        "  }",
        "  public static String allSql='select\\n'+",
        ...
        "  '\\n';",
        "  public static List<PostCodeMainPage_postcodeMaps> get(Connection connection, Messages msgs, String postcode) throws SQLException {",
