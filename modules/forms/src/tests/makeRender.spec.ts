...
describe ( " listComponentsIn", () => {
  ...
  it ( "should createAllReactComponents ", () => {
    expect ( createAllReactComponents ( paramsForTest, AllGuardCreator, makeButtons (), EAccountsSummaryPD, EAccountsSummaryPD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      ...
      "import {ModalCommitWindowButton} from '@focuson/pages';",
      ...
      "  return<>",
      "<>",
      "    <LabelAndNumberInput id={`${id}.accountId`} state={state.focusOn('accountId')} mode={mode} label='Account Id' allButtons={allButtons} required={true} parentState={state} min={10000000} max={99999999} />",
