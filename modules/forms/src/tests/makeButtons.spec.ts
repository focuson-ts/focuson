...
describe ( "makeButtons", () => {
  ...
  it ( "should create modal buttons with copy on close", () => {
    expect ( makeButtonsFrom ( paramsForTest, AllGuardCreator, makeButtons (), OccupationAndIncomeSummaryPD, OccupationAndIncomeSummaryPD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      ...
      "      copy={[{'from':'#selectedOccupation/occupation','to':'~/searchList/search'},{'from':'#selectedOccupation/occupation','to':'~/searchList/selectedOccupationName'}]}",
      "      copyOnClose={[{'from':'~/searchList/selectedOccupationName','to':'#selectedOccupation/occupation'}]}",
      "    />,",
      "    mainOrJoint:<ToggleButton id={`${id}.mainOrJoint`} state={fullPageState.focusOn('mainOrJoint')}",
