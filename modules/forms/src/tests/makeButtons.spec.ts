...
describe ( "makeButtons", () => {
  it ( "should generate a tsx line using that button", () => {
    expect ( makeButtonsFrom ( paramsForTest, AllGuardCreator, makeButtons (), EAccountsSummaryPD,EAccountsSummaryPD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      "    {amendExistingPlan:<ModalButton id={`${id}.amendExistingPlan`} text='Amend Existing Plan'  state={state} modal = 'CreatePlan'  ",
      "      pageMode='edit'",
      ...
      "    />,",
      "    createNewPlan:<ModalButton id={`${id}.createNewPlan`} text='Create New Plan'  state={state} modal = 'CreatePlan'  ",
      "      pageMode='create'",
      "      focusOn='~/tempCreatePlan'",
      "      buttonType={'secondary'}",
  ...
  it ( "should create modal buttons with copy on close", () => {
    expect ( makeButtonsFrom ( paramsForTest, AllGuardCreator, makeButtons (), OccupationAndIncomeSummaryPD,OccupationAndIncomeSummaryPD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      "    {addEntry:<ModalButton id={`${id}.addEntry`} text='Add Entry'  state={state} modal = 'OccupationIncomeModal'  ",
      "      pageMode='create'",
      "      focusOn='~/temp'",
      ...
      "    />,",
      "    additionalInfo:<ModalButton id={`${id}.additionalInfo`} text='Additional Info'  state={state} modal = 'AdditionalInformationModal'  ",
      "      pageMode='edit'",
      "      focusOn='~/additionalInformation'",
      "    />,",
      "    businessDetails:<ModalButton id={`${id}.businessDetails`} text='Business Details'  state={state} modal = 'BusinessDetailsModal'  ",
      "      pageMode='edit'",
      ...
      "    otherSourcesOfIncome:<ModalButton id={`${id}.otherSourcesOfIncome`} text='Other Sources Of Income'  state={state} modal = 'OtherSourcesOfIncomeModal'  ",
      "      pageMode='edit'",
