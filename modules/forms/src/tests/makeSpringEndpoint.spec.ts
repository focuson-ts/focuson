...
describe ( "makeSpringEndpoint", () => {
  ...
  it ( "should make an endpoint for a rest", () => {
    expect ( makeSpringEndpointsFor ( paramsForTest, EAccountsSummaryPD, 'eAccountsSummary', eAccountsSummaryRestD ) ).toEqual ( [
      ...
      "          return Transform.result(connection,graphQL.get(dbName),EAccountsSummaryQueries.getEAccountsSummary(accountId, applRef, brandRef, clientRef, dbName, employeeType), \"getEAccountsSummary\", msgs);",
      "        }",
      "    }",
      "",
  ...
  it ( "should make a second endpoint for a res", () => {
    expect ( makeSpringEndpointsFor ( paramsForTest, EAccountsSummaryPD, 'createPlanRestD', createPlanRestD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      ...
      "        Messages msgs = Transform.msgs();",
      "        try (Connection connection = dataSource.getConnection()) {",
      "          return Transform.result(connection,graphQL.get(IFetcher.mock),CreatePlanQueries.getCreatePlan(accountId, applRef, brandRef, clientRef, createPlanId), 'getCreatePlan', msgs);",
      "        }",
      "    }",
      "",
      ...
      "        Messages msgs = Transform.msgs();",
      "        try (Connection connection = dataSource.getConnection()) {",
      "          return Transform.result(connection,graphQL.get(IFetcher.mock),CreatePlanQueries.createCreatePlan(accountId, applRef, brandRef, clientRef,   Transform.removeQuoteFromProperties(body, Map.class)), 'createCreatePlan', msgs);",
      "        }",
      "    }",
      "",
      ...
      "        Messages msgs = Transform.msgs();",
      "        try (Connection connection = dataSource.getConnection()) {",
      "          return Transform.result(connection,graphQL.get(IFetcher.mock),CreatePlanQueries.updateCreatePlan(accountId, applRef, brandRef, clientRef, createPlanId,   Transform.removeQuoteFromProperties(body, Map.class)), 'updateCreatePlan', msgs);",
      "        }",
      "    }",
      "",
  ...
  it ( "should make an endpoint for a repeating", () => {
    expect ( makeSpringEndpointsFor ( paramsForTest, RepeatingPageD, 'repeating', repeatingRestRD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      ...
      "        Messages msgs = Transform.msgs();",
      "        try (Connection connection = dataSource.getConnection()) {",
      "          return Transform.result(connection,graphQL.get(IFetcher.mock),RepeatingWholeDataQueries.createRepeatingLine(clientRef,   Transform.removeQuoteFromProperties(body, List.class)), 'createRepeatingLine', msgs);",
      "        }",
      "    }",
      "",
      ...
      "        Messages msgs = Transform.msgs();",
      "        try (Connection connection = dataSource.getConnection()) {",
      "          return Transform.result(connection,graphQL.get(IFetcher.mock),RepeatingWholeDataQueries.getRepeatingLine(clientRef), 'getRepeatingLine', msgs);",
      "        }",
      "    }",
      "",
  ...
  it ( "should make spring boot endpoints when no parameters", () => {
    expect ( makeSpringEndpointsFor ( paramsForTest, PostCodeMainPage, 'address', addressRestD ).map ( s => s.replace ( /"/g, "'" ) ) ).toEqual ( [
      ...
      "        Messages msgs = Transform.msgs();",
      "        try (Connection connection = dataSource.getConnection()) {",
      "          return Transform.result(connection,graphQL.get(IFetcher.mock),PostCodeNameAndAddressQueries.createPostCodeNameAndAddress(  Transform.removeQuoteFromProperties(body, Map.class)), 'createPostCodeNameAndAddress', msgs);",
      "        }",
      "    }",
      "",
