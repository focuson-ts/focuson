import { identityOptics, Optional } from "@focuson-nw/lens";
import { applyFetcher, Fetcher } from "./fetchers";
import { FetcherDebug, wouldLoadSummary } from "./setjson";
import { defaultFetchFn, FetchFn } from "@focuson-nw/utils";
...
export async function loadTree<State> ( fg: FetcherTree<State>, ns: State, fetchFn?: FetchFn, debug?: FetcherDebug ): Promise<State> {
  ...
  const initialValue: State = await fg.fetchers.reduce ( ( acc, pf ) => acc.then ( s => applyFetcher ( pf, s, realFetch, debug ) ), Promise.resolve ( ns ) )
  return fg.children.reduce<Promise<State>> ( loadGraphChildLink ( realFetch, debug ), Promise.resolve ( initialValue ) )
}
