...
import {AdditionalInfoFirst} from '../MainOccupationDetailsPageSummary/MainOccupationDetailsPageSummary.render'
...
export function AdditionalInfoFirstModalPage(){
  return focusedPage<FState, AdditionalInfoFirstDomain, Context> ( s => '' ) (//If there is a compilation here have you added this to the 'domain' of the main page
     ( state, d, mode, index ) => {
          const id=`page${index}`;
          const buttons =    {cancel:<ModalCancelButton id={`${id}.cancel`} state={state} />,
              commit:<ModalCommitButton id={`${id}.commit`}  state={state} />,}
          return <>
          <AdditionalInfoFirst id={`${id}`} state={state} mode={mode} buttons={buttons} />
          { buttons.cancel } 
          { buttons.commit } 
          </>})}