...
//If you have a compilation error because of duplicate names, you need to give a 'namePrefix' to the offending restDs
export function ListOfPaymentsPage_somePrefix_PrintRecordHistoryRestDetails ( cd: NameAndLens<FState>, dateFn: DateFn  ): OneRestDetails<FState, domains.ListOfPaymentsPagePageDomain, domains.PrintRecordHistoryDomain, SimpleMessage> {
  const fdd: NameAndLens<domains.ListOfPaymentsPagePageDomain> = {}
  return {
    fdLens: Lenses.identity<FState>().focusQuery('ListOfPaymentsPage'),
//From ListOfPaymentsPage.rest[paymentHistoryNext].targetFromPath (~/display). Does the path exist? Is the 'type' at the end of the path, the type that rest is fetching?
    ...
    states : {"print":{"url":"/api/printrecord/print?{query}"}}
  }
}
