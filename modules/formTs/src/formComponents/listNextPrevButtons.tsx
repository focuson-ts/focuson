...
export function ListNextButton<S, C> ( { id, title, value, list, enabledBy }: ListButtonProps<S, C> ) {
...
export function ListPrevButton<S, C> ( { id, title, value, enabledBy }: ListButtonProps<S, C> ) {
  const index = value.optJson ()
  const i = index ? index : 0
