...
const ownShareOfTheCompanyGuard = state.focusOn('ownShareOfTheCompany').optJson();
  return <Layout details='[[3,3],[{"count":5,"labelWidth":70, "valueWidth":40},5],[30]]' title='Current employment details - '>
    ...
    <Guard value={areYouGuard} cond={["E"]}><Guard value={employmentTypeGuard} cond={["1"]}><LabelAndDateInput id={`${id}.empStartDate`} state={state.focusOn('empStartDate')} mode={mode} label='When did this employment start? (mm/yyyy) ' allButtons={buttons} /></Guard></Guard>
    <Guard value={areYouGuard} cond={["E"]}><Guard value={employmentTypeGuard} cond={["2","3"]}><LabelAndDateInput id={`${id}.empEndDate`} state={state.focusOn('empEndDate')} mode={mode} label='When will it finish? (mm/yyyy) ' allButtons={buttons} /></Guard></Guard>
    ...
    <LabelAndDateInput id={`${id}.createdDate`} state={state.focusOn('createdDate')} mode={mode} label='on ' allButtons={buttons} />
</Layout>
}
