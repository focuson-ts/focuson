...
export function OverpaymentHistoryLine({id,state,mode,buttons}: FocusedProps<FState, OverpaymentHistoryLineDomain,Context>){
  return <>
    <LabelAndNumberInput id={`${id}.amountReceived`} state={state.focusOn('amountReceived')} mode={mode} label='Amount Received' allButtons={buttons} required={true} />
    <LabelAndDateInput id={`${id}.date`} state={state.focusOn('date')} mode={mode} label='Date' allButtons={buttons} />
    <LabelAndDropdown id={`${id}.status`} state={state.focusOn('status')} mode={mode} label='Status' allButtons={buttons} enums={{"COLLECTED":"COLLECTED","CANCELLED":"CANCELLED"}} />
</>
}

export function OverpaymentPage({id,state,mode,buttons}: FocusedProps<FState, OverpaymentPageDomain,Context>){
  return <>
    <Table id={`${id}.history`} state={state.focusOn('history')} mode={mode} order={["amountReceived","date","status"]} />
    <LabelAndDateInput id={`${id}.drawDownDate`} state={state.focusOn('drawDownDate')} mode={mode} label='Draw Down Date' allButtons={buttons} />
    <LabelAndNumberInput id={`${id}.initialBorrowing`} state={state.focusOn('initialBorrowing')} mode={mode} label='Initial Borrowing' allButtons={buttons} required={true} />
</>
}
