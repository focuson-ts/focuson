...
export function ExcessHistoryPage(){
  return focusedPage<FState, AccountOverviewHistoryDomain, Context> ( s =>  'Excess History' ) (//If there is a compilation here have you added this to the 'domain' of the main page
     ( state, d, mode, index ) => {
          const id=`page${index}`;
          const allButtons =    {cancel:<ModalCancelButton id={`${id}.cancel`} state={state} />,
              ...
              />,}
          return <>
          <AccountOverviewHistory id={`${id}`} state={state} mode={mode} label='' allButtons={allButtons} />
          { allButtons.cancel } 
          { allButtons.commit } 
          { allButtons.details } 
          </>})}